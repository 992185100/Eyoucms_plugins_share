...
//			session_start();
    ...
    public function creat(){
        $imagePathArr = array('./weapp/Clicap/vendor/image/1.jpg', './weapp/Clicap/vendor/image/2.jpg', './weapp/Clicap/vendor/image/3.jpg');
        $imagePath = $imagePathArr[rand(0, count($imagePathArr) - 1)];
        $fontPath = realpath('./weapp/Clicap/vendor/fonts/SourceHanSansK-Regular.ttf');
        /***自行添加 start***/
        $clicap = Db::name('weapp')->where('code','Clicap')->find();
        if (!empty($clicap['data']) && $clicap['data'] != "[]" ) {
            $clicap['data'] = unserialize($clicap['data']);
            $conf_cap = $clicap['data']['captcha']['admin_login']['config']['verification'];
            if (!empty($conf_cap)){
                $verification = explode("\r\n",$conf_cap);
                //判断验证码有多少组
                $c_num = mt_rand(0, count($verification)-1);
                $input_value = $verification[$c_num];
                if (!empty($input_value)){
                    $input_arr = [];
                    for ($i=0; $i<mb_strlen($input_value, 'utf-8'); $i++){
                        $input_arr[] = mb_substr($input_value, $i, 1, 'utf-8');
                    }
                }else{
                    //原来取值方法
                    $input_arr = $this->randChars(6);
                }
            }else{
                //原来取值方法
                $input_arr = $this->randChars(6);
        ...
        $textArr['text'] = array_splice($textArr['text'], 0, 4);
        $text = array_splice($text, 0, 4);
        $_SESSION['clicaptcha_text'] = $textArr;
        cookie('clicaptcha_text', implode(',', $text), time() + 3600, '/');
        //生成图片
        switch($imageType){
            case 1://GIF
    ...
    private function randChars($length = 4){
         ...
         * @return string
         */
        function c_msubstr($str, $start = 0, $length, $charset = 'utf-8', $suffix = true){
        ...
        for($i = 0; $i < $length; $i++){
            $return[] = c_msubstr($chars, floor(mt_rand(0, mb_strlen($chars, 'utf-8') - 1)), 1, 'utf-8', false);
        }
        return $return;
    }
    //随机生成位置布局
